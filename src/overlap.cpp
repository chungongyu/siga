 ...
 public:
  int run(const Properties& options, const Arguments& arguments) {
    ...
    if (FMIndex::load(output + BWT_EXT, fmi) && FMIndex::load(output + RBWT_EXT, rfmi)) {
      ...
      if (!builder.build(input, options.get<size_t>("min-overlap", 10), output + ASQG_EXT + GZIP_EXT, options.get<size_t>("threads", 1), options.get<size_t>("batch-size", 10000))) {
...
static const option longopts[] = {
    ...
    {"exhaustive",          no_argument,        nullptr, 'x'},
