...
static log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("arcs.Scaffold"));

class Scaffold : public Runner {
public:
    int run(const Properties& options, const Arguments& arguments) {
        ...
        LOG4CXX_DEBUG(logger, "scaffold reads begin");
        ...
        LOG4CXX_DEBUG(logger, "scaffold reads end");
        return r;
    }
...
private:
    Scaffold(const std::string& name, const std::string& description, const std::string& shortopts, const option* longopts) : Runner(shortopts, longopts) {
        RUNNER_INSTALL(name, this, description, kScaffold);
    }
    ...
    int printHelps() const {
        std::cout << "arcs scaffold -o [output] -e [read_cutoff] -n [buckets] [<inputs>]" << std::endl;
        return 256;
    }
